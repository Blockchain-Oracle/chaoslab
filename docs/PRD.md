...
**Direct competitive cut:** AIUC ($15M seed, the category leader) sells quarterly enterprise audits + an insurance certificate written by the same shop that did the audit. Phoenix Audit sells continuous, self-serve, signed evidence keyed to the Customer's OWN compliance officer's Cloud KMS key — and ships a **hybrid Phoenix-hosting model** (ADR-017): the **zero-friction default** runs Phoenix Audit-hosted with a 24-hour retention SLA + cryptographic-erasure deletion (Phoenix Audit acts as a GDPR Article 28 data processor for that window; see `docs/data-retention-policy.md`), AND a **BYO-key mode** for regulated industries where audit traces stay in the Customer's Phoenix project and Phoenix Audit holds no copy after the audit run (the original ADR-013 contract preserved verbatim for BYO mode). Zero auditor/insurer conflict of interest in both modes. See `research/google-cloud-rapid-agent/brainstorm/19-ai-agent-governance-competitive-landscape.md` for the full landscape.
...
- **Phoenix Cloud free tier is 25,000 spans/month** (~250 spans per 6-test audit, ~100 audits/month ceiling). Per ADR-017 (hybrid Phoenix-hosting), Phoenix Audit's **default mode runs self-hosted Phoenix in Postgres-backed Docker** (`infra/phoenix-self-host/`) which has no quota and is empirically 6× faster (0.20s emit-to-visible vs Arize Cloud's 1.37s, measured in `/tmp/phoenix-self-host-smoke-test.md`). Phoenix Cloud is OPTIONAL infrastructure available to Customers who pick BYO-key mode for regulatory reasons.
...
- **Customer-runs-their-own-Phoenix-Audit-instance behind their firewall** — that's the post-hackathon enterprise self-host path. Default-mode Phoenix Audit DOES self-host the Phoenix backend per ADR-017 (`infra/phoenix-self-host/`), but the orchestrator + web UI are still Phoenix Audit-hosted in default mode; full air-gapped Phoenix Audit deploys are out of scope.
