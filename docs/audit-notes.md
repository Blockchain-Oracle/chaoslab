...
**Adjacent bug.** The auditor's `TARGET_PHOENIX_PROJECT=phoenix-audit` env (from a 2026-06-10 belief that Phoenix routes child spans into the parent's project) was wrong — Phoenix routes by the **producing tracer's project resource attribute**, not by trace_id. Once target spans started flowing, the judge still saw nothing because it was querying the wrong project. Flipped to `TARGET_PHOENIX_PROJECT=target-agent`.
