...
### IF-19 — Cloud Run default CPU throttling drops Phoenix `SimpleSpanProcessor` exports (2026-06-11, target-agent)

**Symptom.** Every probe in every staging audit erred with `fault X was registered but its execution marker never appeared in trace Y` — the auditor saw only its own 5 wire spans per trace_id, never a single target-side span. The `target-agent` Phoenix project went silent 2026-06-10 19:24 UTC and stayed silent for 24h+ while the deployed audit pipeline ran ~hourly. Boot logs showed `phoenix_observability_setup` succeeded with the correct project + API key + endpoint, and the live target processed POST `/` requests cleanly (200 OK, ADK runner produced complete responses). Pythonic-end: 0 spans visible anywhere.

**Root cause.** Phoenix's `register()` installs a `SimpleSpanProcessor` (despite the `batch=True` kwarg in the architecture comment — the boot banner is the source of truth: `Span Processor: SimpleSpanProcessor`). SimpleSpanProcessor exports each span synchronously via a blocking HTTPS POST inside `on_end()`. ADK's `agent_run` / `call_llm` / `execute_tool` spans END right around the time the HTTP response is being flushed to the client. With Cloud Run's default `cpu-throttling=true`, CPU drops to ~5% the moment the request returns, the in-progress OTLP POST stalls, and the span is silently dropped. No log line is emitted because the SDK never sees a connect-error response — the socket just hangs until the SDK gives up internally.

**Why the auditor service was unaffected.** Its only listener is the SSE `/stream` endpoint, which holds the request open for the full ~90s audit duration. CPU stays allocated for that entire window, so its synchronous OTLP POSTs always complete inside the request window. The target serves short discrete probes (~5s each) followed by idle gaps — exactly the throttle-vulnerable shape.

**Reproduction (proof).** Local target with the SAME image + env on a non-throttled host emitted 50 spans for a single A2A POST in the `target-agent` Phoenix project, including the `phoenix_audit.fault.type` marker. The live revision with `--no-cpu-throttling` likewise emitted 48 spans for an identical curl POST. The same revision with default throttling emitted 0.

**Fix.** `extra_flags: --no-cpu-throttling` on the target-agent matrix entry in `.github/workflows/staging-deploy.yaml`, wired through to `gcloud run deploy`. Applied to the live service directly via `gcloud run services update target-agent --no-cpu-throttling`. The auditor and web stay default-throttled (auditor doesn't need it; web has no instrumentation hot path).

**Adjacent observation (not a separate bug).** While diagnosing, I briefly flipped `TARGET_PHOENIX_PROJECT` from `phoenix-audit` → `target-agent` on a wrong theory ("Phoenix routes spans by the producing tracer"). Re-verified after the BatchSpanProcessor + ForceFlush fix made target spans actually visible: Phoenix Cloud routes child spans to whichever project owns the trace_id's root span. Two probe traces (`3043eca1...` and `c72c3e3b...` from `run_fddd5de82845`) had target-side `agent_run` / `execute_tool` / `tool.lookup_order` spans land in the auditor's `phoenix-audit` project, not the target's own resource project — because the auditor's `phoenix-audit.adapter.adk.invoke` span was the trace_id's root. Reverted env back to `TARGET_PHOENIX_PROJECT=phoenix-audit`. NOTE: re-verify under full audit volume after PR #121 lands; if the routing claim doesn't hold for all 13 traces, the env may need to be set per-probe or both projects queried.

**Rule for future Cloud Run + OTel stacks.** Always pair `SimpleSpanProcessor` (Phoenix's default) with `--no-cpu-throttling`, OR switch to `BatchSpanProcessor` + explicit `force_flush()` in a request-bound shutdown path. The default Cloud Run + default Phoenix combo is a silent-failure pit.
...
**Local-seeding gotchas:** user ADC cannot mint v4 signed URLs or KMS-sign (no private key, no `sign_bytes`) — run seeds with an impersonated-SA ADC file (`type: impersonated_service_account` wrapping the user ADC; requires `roles/iam.serviceAccountTokenCreator` on chaoslab-runtime, the B8 self-grant). WeasyPrint on macOS additionally needs `DYLD_FALLBACK_LIBRARY_PATH=/opt/homebrew/lib` (the conftest sets it for tests only — a bare seed run skips the PDF with a `libgobject` error).

**Two more found at verification (PRs #104/#105):** Cloud Run's token-only credentials make a bare `blob.generate_signed_url(v4)` raise AttributeError — EVERY deployed run had dead artifact URLs since deploy day; signing must go through the IAM signBlob kwargs. And the signBlob auth token cannot be the storage client's own (devstorage-scoped → 403 "insufficient authentication scopes") — mint a separate cloud-platform-scoped ADC token. SDK shape changes also broke the judge/patcher (`generate_content` rejects bare `temperature=`; `HallucinationEvaluator` wants `context` not `reference`; openinference flattens `llm.tools`) — fixed in PR #103 with strict-adapter test fakes per the IF-15 rule.
...
**Fix shape (PR "evidence-chain repair"):** faults export JSON descriptors (`injector/faults/descriptors.py`) carrying the resolved payload; the target-agent gains an env-gated hook surface (`target_agent/fault_hooks.py`, `POST/DELETE /hooks/adk`, FAULT_HOOKS_ENABLED) that installs the equivalent callback server-side per registration; the ADK adapter delivers descriptors through the existing `webhook_fault_session` machinery, injects W3C `traceparent` (httpx event hook), and reports the 32-hex trace id; the target extracts trace context via a 20-line ASGI middleware so its spans join the auditor's trace; `judge_phase` prefetches each probe's trace ONCE (shared honored-check + rubric) and runs probes concurrently (Semaphore(8)); auditor-originated data (attack payload, original prompt, client duration) rides `RubricInput` directly instead of round-tripping through span attributes. The demo target gained a real `search_policy_kb` BaseRetrievalTool so F3 retriever_insert probes are genuine.

**Rules this memorializes:** (1) a contract that only exists in test fakes is not a contract — every cross-process key needs a producer-side test plus a composition test of the real classes together (`tests/unit/test_cross_app_fault_contract.py`); (2) auditor-known data must not be "read back" from spans — spans carry TARGET evidence only; (3) an undeliverable fault must RAISE (FaultDeliveryError), never fall back to a no-fault invocation that inflates the pass rate.
