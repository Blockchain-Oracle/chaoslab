...
RAT-2 Risk A. The mix is documented here so a compliance officer reading the
...
handles session statefulness:

| Tool                    | Default                                                                                                                                                                                                                                                 |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Garak**               | Stateless per probe — "Prototype, mostly stateless."                                                                                                                                                                                                    |
...
**Garak's stateless-per-probe default is the trap we don't want to fall into.**
If Phoenix Audit silently ships 6 single-turn probes and calls it
"comprehensive," a compliance officer comparing our report to Promptfoo's or
DeepTeam's would notice within 5 minutes that the same OWASP LLM01 category
...
- **vs Garak (stateless):** Phoenix Audit emits 2-turn probes for 3 of 6 tests,
  catching the Crescendo-class attacks Garak's defaults cannot.
...
story pins the exact AML.Txxxx IDs against the MITRE ATLAS v5.1.0 dataset.
This spec doc declares the SHAPE (single-turn vs 2-turn) per probe slot,
not the dataset-row binding (which is implementation detail).

**2-turn protocol (locked):** for each 2-turn probe, turn 1 establishes
context (benign question, plausible request) and turn 2 escalates (injection
payload, trust exploitation). Each turn is a full A2A round-trip to the
target; total wire time per 2-turn probe ≈ 32s.
...
Empirical baseline from RAT-2 Risk A: **16s per A2A round-trip** at current
ADK 2.1.0 wire performance (no-LLM, localhost). Connection reuse doesn't
help; concurrent makes it worse (target-side serialization).
...
- Concurrent execution (Cloud Run concurrency = ~10): wall-clock drops to
  ~32s (limited by the slowest 2-turn probe).

The 90-second demo budget assumes concurrent execution + judge/patcher/
reporter operations overlapping with probe-emission. If concurrency
degrades, the demo overruns; the failure mode is "longer demo," not
"silently incorrect audit."
...
- ANTI-anchor: the report MUST NOT claim "comprehensive coverage" of any
  attack category; the framing is "6-probe sample under deliberate
...
- `research/google-cloud-rapid-agent/RAT-2-results.md` Risk A — the 16s
...
- `docs/PRD.md` Known limitations §"A2A round-trip latency" — the same
  empirical constraint as user-facing language
