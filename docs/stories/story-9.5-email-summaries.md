...
- **Given** a schedule with `deliver_email=True` fires and its run finalizes, **then** the finalize path sends a summary email to the owner's profile email containing: target URL, pass/fail/error tally, and a fresh-signed report link when `report_available` (link via the existing `sign_blob_url` seam). **Given** `deliver_email=False`, **then** no send is attempted. **Given** the owner profile has no email, **then** the send is skipped with a structured `schedule_email_skipped_no_address` warning. **Given** Resend raises, **then** the finalize still completes — the failure is logged, never propagated.
...
- **Given** the PDF exceeds the attachment cap (30 MB; Resend's hard limit is 40 MB per message), **then** the email still sends link-only and the response discloses `"attachment_included": false` — the fallback is visible, never silent.
...
- Web: `apps/phoenix-audit-web/app/api/agent/[...path]/route.ts` — allowlist `+/^runs\/[a-zA-Z0-9_-]+\/email$/`; `components/artifacts/email-report-button.tsx` (NEW — idle/sending/sent/failed states) mounted on the report page next to the download actions.
- Tests: `apps/phoenix-audit-agent/tests/unit/notifier/test_email.py` (NEW), `tests/unit/notifier/test_report_mail.py` (NEW), `tests/unit/api/test_runs_email_api.py` (NEW), schedule-id threading asserts in a NEW `tests/unit/test_schedule_id_threading.py`; web `tests/proxy-allowlist.test.ts` (extend) + `tests/email-report-button.test.tsx` (NEW).
