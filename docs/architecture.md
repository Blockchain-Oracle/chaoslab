...
### ADR-011: Hybrid GitLab MR emission — official MCP for MR creation, `python-gitlab` SDK for branch + file commits (AMENDED 2026-06-03 per audit A1; AMENDED 2026-06-11 per story-9.17 — opt-in review-first + per-user OAuth)

**Decision (AMENDED 2026-06-11):** The MR is filed **opt-in, review-first, with the user's own GitLab OAuth identity** — never automatically, never with a shared service token. The pipeline emits (a) the Markdown artifact to GCS plus a `{recipe_id}.json` sidecar (the structured recipe, so the MR endpoint can rehydrate it after the run). A human reviews the recipe, clicks "File as GitLab MR", and picks a target project; only then do (b) GitLab branch creation via `python-gitlab` SDK, (c) file commits via `python-gitlab` SDK, and (d) **MR creation via the official `https://gitlab.com/api/v4/mcp` endpoint's `create_merge_request` tool** run — all authenticated with the USER's OAuth token (`scope=api`, PKCE connect per story-9.17). The MCP call is unchanged — this is still the call that earns the hackathon's "official MCP" judging credit. The service-env `gitlab_token` survives ONLY as the seed-script/internal fallback; the user-facing path has NO fallback from user OAuth to service token (filing as the wrong identity is worse than failing). Rationale for the 2026-06-11 amendment: Abu's explicit 2026-06-10 decision — a Director of AI Governance will not point a product at their repo if it can write to it autonomously under someone else's identity. The MR only ADDS files under `phoenix-audit/`.
