...
3. **Davis AI causal RCA.** Where Prometheus/Grafana shows you "this metric spiked," Davis shows you "this metric spiked *because* a deploy 4 minutes ago changed config X on service Y which is upstream of Z" — actual causal graph.
...
**Why Dynatrace:** `list_vulnerabilities` knows the *runtime* exposure (which JARs are actually loaded), not just what's in `package.json`. Agent cross-references with deployment metadata, opens triage workflow.
