...
- **No file >400 lines** in `apps/`, `packages/`, `scripts/` (Python, TS, JSX, Markdown — extensions per `scripts/check_max_lines.py`). Split before 350. Skipped: `__init__.py`, `.d.ts`, `_vendored/`, generated dirs. `docs/` is exempt (spec completeness > brevity for orchestrator + coding-agent comprehension).
