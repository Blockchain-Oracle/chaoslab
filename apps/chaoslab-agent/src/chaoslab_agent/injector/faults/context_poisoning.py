...
_INSTALLED_SENTINEL = "_chaoslab_f3_installed"
_PATCHED_SENTINEL = "_chaoslab_f3_patched"
...
def _make_patched_run_async(original: Any, poison: str) -> Any:
    async def patched(*, args: dict[str, Any], tool_context: Any) -> Any:
        ...
        span.set_attribute("chaoslab.fault.injected", False)
        span.set_attribute(
            "chaoslab.fault.skipped_reason", f"unsupported_type:{type(result).__name__}"
        )
        msg = (
