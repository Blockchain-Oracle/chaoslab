...
async def test_falls_back_to_input_value_when_chaoslab_original_missing(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    captured: list[dict[str, Any]] = []
    monkeypatch.setattr(f2, "_EVALUATOR", stub_evaluator(StubVerdict("PASS"), captured=captured))
    attrs = _full_attrs()
    del attrs["chaoslab.original_user_message"]
...
@pytest.mark.parametrize("missing", ["chaoslab.payload", "output.value"])
