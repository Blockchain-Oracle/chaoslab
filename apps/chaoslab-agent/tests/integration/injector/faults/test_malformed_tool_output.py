...
``chaoslab.fault.type`` / ``chaoslab.fault.mode`` attributes plus the
...
async def test_invalid_json_mode_sets_span_attrs_and_returns_unparseable_payload(
    ...
    payload = result["_chaoslab_malformed_payload"]
...
async def test_missing_required_field_mode_omits_total(
    ...
    span = _last_tool_span(exporter)
    assert span.attributes is not None
    assert span.attributes.get("chaoslab.fault.mode") == "missing_required_field"
...
async def test_type_mismatch_mode_swaps_field_types(
    ...
    span = _last_tool_span(exporter)
    assert span.attributes is not None
    assert span.attributes.get("chaoslab.fault.mode") == "type_mismatch"
...
async def test_target_tool_name_mismatch_lets_real_tool_run(
    ...
    assert span.attributes.get("chaoslab.fault.mode") is None
