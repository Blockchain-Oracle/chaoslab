"""FastAPI endpoint tests for phoenix-audit-agent."""
...
from phoenix_audit_agent.config import JUDGE_LLM_LOCKED, get_settings
...
def app():
    ...
    from phoenix_audit_agent.main import app as fastapi_app
...
async def test_post_run_persists_run_id_to_registry(client: httpx.AsyncClient) -> None:
    ...
    from phoenix_audit_agent.main import _RUN_REGISTRY
...
async def test_cancel_task_cancels_in_flight_drive() -> None:
    ...
    """
    from phoenix_audit_agent.main import _RUN_TASKS, _cancel_task
...
async def test_real_drive_failure_emits_error_frame_and_logs(
    ...
    with caplog.at_level(_logging.ERROR, logger="phoenix_audit_agent.main"):
        run_resp = await client.post("/run", json={"target_url": "http://localhost:8001"})
        run_id = run_resp.json()["run_id"]
        chunks: list[str] = []
        async with client.stream("GET", f"/stream?runId={run_id}") as resp:
...
def test_run_uvicorn_honors_port_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from phoenix_audit_agent.main import run_uvicorn
...
def _patch_lifespan_deps(monkeypatch: pytest.MonkeyPatch, emitter_cls: type) -> None:
    ...
    so we also patch `phoenix_audit_agent.main.MarkdownEmitter` (raising=False because
    that name doesn't exist at module top today; the patch survives a hoist).
    """
    import phoenix_audit_agent.main as main_mod
    import phoenix_audit_agent.observability as obs
    import phoenix_audit_agent.patcher.markdown_emitter as me
...
async def test_lifespan_invokes_markdown_emitter_health_check(
    ...
    from phoenix_audit_agent.main import _lifespan
    from phoenix_audit_agent.main import app as fastapi_app
...
async def test_lifespan_skips_health_check_when_probe_disabled(
    ...
    with caplog.at_level(_logging.CRITICAL, logger="phoenix_audit_agent.main"):
        async with _drive():
            yielded["ok"] = True
...
def test_settings_env_prod_forbids_disabled_probe(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from phoenix_audit_agent.config import Settings
...
def test_gcs_probe_env_name_constant_matches_settings_field() -> None:
    ...
    from phoenix_audit_agent.config import GCS_PROBE_ENV_NAME, Settings
