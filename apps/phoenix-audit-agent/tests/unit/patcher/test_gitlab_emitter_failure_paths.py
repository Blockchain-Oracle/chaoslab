...
from phoenix_audit_agent.patcher.recipe import FailureCluster, HardeningRecipe
...
async def test_emit_rolls_back_branch_when_mcp_fails() -> None:
    ...
    async with httpx.AsyncClient(timeout=5.0) as http:
        mcp = GitLabMcpClient(endpoint=OFFICIAL_ENDPOINT, token="tok", client=http)
        emitter = GitLabMREmitter(rest_client=rest, mcp_client=mcp)
        with pytest.raises(GitLabEmitterError, match="phoenix-audit/recipe-abc123def456"):
            await emitter.emit(make_recipe(), project_id="abu-phoenix-audit/test-target")

    assert project.branches.delete.call_count == 1
    assert project.branches.delete.call_args[0][0] == "phoenix-audit/recipe-abc123def456"
