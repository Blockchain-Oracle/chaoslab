...
async def test_posts_create_merge_request_tool_with_arguments() -> None:
    ...
    async with httpx.AsyncClient(timeout=5.0) as http:
        client = GitLabMcpClient(endpoint=OFFICIAL_ENDPOINT, token="tok", client=http)
        await client.create_merge_request(
            project_id="abu-phoenix-audit/test-target",
            source_branch="phoenix-audit/recipe-abc123def456",
            target_branch="main",
            title="title",
            description="desc",
            labels=["phoenix-audit"],
        )
    assert route.called
    body = json.loads(route.calls.last.request.content)
    assert body["params"]["name"] == "create_merge_request"
    assert body["params"]["arguments"]["source_branch"] == "phoenix-audit/recipe-abc123def456"
