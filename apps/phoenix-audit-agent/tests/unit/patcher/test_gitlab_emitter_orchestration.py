...
from phoenix_audit_agent.patcher.recipe import RegressionTestCase, ToolValidationDiff
...
def test_patcher_init_exports_emitter() -> None:
    ...
    from phoenix_audit_agent import patcher
...
async def test_emit_creates_branch_via_rest_client() -> None:
    ...
    from phoenix_audit_agent.patcher._gitlab_rest_client import GitLabRestClient
    from phoenix_audit_agent.patcher.gitlab_emitter import GitLabMREmitter
    ...
    async with httpx.AsyncClient(timeout=5.0) as http:
        ...
        await emitter.emit(make_recipe(), project_id="abu-phoenix-audit/test-target")

    assert project.branches.create.call_count == 1
    call_kwargs = project.branches.create.call_args[0][0]
    assert call_kwargs["branch"] == "phoenix-audit/recipe-abc123def456"
...
async def test_emit_commits_files_with_create_actions() -> None:
    ...
    async with httpx.AsyncClient(timeout=5.0) as http:
        mcp = GitLabMcpClient(endpoint=OFFICIAL_ENDPOINT, token="tok", client=http)
        emitter = GitLabMREmitter(rest_client=rest, mcp_client=mcp)
        result = await emitter.emit(recipe, project_id="abu-phoenix-audit/test-target")

    payload = project.commits.create.call_args[0][0]
    assert payload["branch"] == "phoenix-audit/recipe-abc123def456"
    actions = payload["actions"]
    file_paths = [a["file_path"] for a in actions]
    assert all(a["action"] == "create" for a in actions)
    assert any(p == "phoenix-audit/patches/recipe_abc123def456.md" for p in file_paths)
...
async def test_emit_returns_emit_result_with_valid_gitlab_mr_url() -> None:
    ...
    from phoenix_audit_agent.patcher._gitlab_mcp_client import GitLabMcpClient
    from phoenix_audit_agent.patcher._gitlab_rest_client import GitLabRestClient
    from phoenix_audit_agent.patcher.gitlab_emitter import GitLabEmitResult, GitLabMREmitter
    ...
    async with httpx.AsyncClient(timeout=5.0) as http:
        mcp = GitLabMcpClient(endpoint=OFFICIAL_ENDPOINT, token="tok", client=http)
        emitter = GitLabMREmitter(rest_client=rest, mcp_client=mcp)
        result = await emitter.emit(make_recipe(), project_id="abu-phoenix-audit/test-target")
    ...
    assert result.branch_name == "phoenix-audit/recipe-abc123def456"
...
async def test_emit_mr_description_contains_recipe_id_and_rendered_markdown() -> None:
    ...
    async with httpx.AsyncClient(timeout=5.0) as http:
        mcp = GitLabMcpClient(endpoint=OFFICIAL_ENDPOINT, token="tok", client=http)
        emitter = GitLabMREmitter(rest_client=rest, mcp_client=mcp)
        await emitter.emit(recipe, project_id="abu-phoenix-audit/test-target")
...
def test_emit_result_rejects_non_https_url() -> None:
    ...
    from phoenix_audit_agent.patcher.gitlab_emitter import GitLabEmitResult

    with pytest.raises(ValueError, match="must match"):
        GitLabEmitResult(
            mr_url="http://gitlab.com/x/-/merge_requests/1",
            mr_iid=1,
            branch_name="phoenix-audit/recipe-abc123def456",
            commit_count=1,
...
def test_emit_result_rejects_invalid_branch_name() -> None:
    """branch_name pattern locks the `phoenix-audit/recipe-<12hex>` shape."""
    from phoenix_audit_agent.patcher.gitlab_emitter import GitLabEmitResult
...
async def test_emit_only_markdown_file_when_no_diffs_or_regressions() -> None:
    ...
    assert actions[0]["file_path"] == f"phoenix-audit/patches/{recipe.recipe_id}.md"
...
async def test_emit_extracts_mr_from_envelope_shape(
                    ...
                    "content": [
                        {
                            ...
                            "json": {
                                "iid": 77,
                                "web_url": (
                                    "https://gitlab.com/abu-phoenix-audit/test-target"
                                    "/-/merge_requests/77"
                                ),
                            },
                        }
    ...
    project = make_fake_project()
    rest = GitLabRestClient(project=project)
    with caplog.at_level(_logging.WARNING, logger="phoenix_audit_agent.patcher.gitlab_emitter"):
        async with httpx.AsyncClient(timeout=5.0) as http:
            mcp = GitLabMcpClient(endpoint=OFFICIAL_ENDPOINT, token="tok", client=http)
            emitter = GitLabMREmitter(rest_client=rest, mcp_client=mcp)
            result = await emitter.emit(make_recipe(), project_id="abu-phoenix-audit/test-target")
