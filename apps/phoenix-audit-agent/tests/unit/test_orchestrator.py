...
def test_build_orchestrator_returns_sequential_agent_instance() -> None:
    from phoenix_audit_agent.orchestrator import build_orchestrator
...
def test_orchestrator_name_is_locked() -> None:
    """`PhoenixAuditOrchestrator` is the span name asserted in trace-as-assertion gates."""
    from phoenix_audit_agent.orchestrator import ORCHESTRATOR_NAME, build_orchestrator

    assert ORCHESTRATOR_NAME == "PhoenixAuditOrchestrator"
...
async def test_orchestrator_emits_three_ordered_agent_spans(  # noqa: PLR0915 — trace-as-assertion test legitimately wires OTel + ADK + assertions
    ...
    try:
        orch = build_orchestrator()
        runner = InMemoryRunner(agent=orch, app_name="phoenix-audit-test")
        session = await runner.session_service.create_session(
            app_name="phoenix-audit-test", user_id="trace-test"
        )
