...
APP_DIR = REPO_ROOT / "apps" / "phoenix-audit-agent"
DOCKERFILE = APP_DIR / "Dockerfile"
IMAGE_TAG = "phoenix-audit-agent:test"
SMOKE_CONTAINER = "phoenix-audit-smoke-test"
...
def test_running_container_uid_is_10001(built_image: str) -> None:
    """`docker exec id -u` returns 10001 (the non-root phoenix-audit uid)."""
    subprocess.run(["docker", "rm", "-f", SMOKE_CONTAINER], capture_output=True, check=False)
    subprocess.run(
        [
            "docker",
