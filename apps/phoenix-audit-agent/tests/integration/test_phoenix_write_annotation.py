...
def test_annotation_result_accepts_valid_payload() -> None:
    from phoenix_audit_agent.phoenix_tools.write_annotation import AnnotationResult

    result = AnnotationResult(
        status="ok",
        span_id=_SPAN_ID,
        annotation_name="phoenix_audit_cluster",
        annotation_identifier="phoenix-audit/0.0.0/default/abcdef0123456789",
        score=0.85,
...
def test_iso_now_has_millisecond_precision() -> None:
    ...
    from phoenix_audit_agent.phoenix_tools.write_annotation import _iso_now
...
def test_phoenix_write_annotation_tool_is_a_function_tool() -> None:
    ...
    from phoenix_audit_agent.phoenix_tools.write_annotation import phoenix_write_annotation_tool
...
async def test_wrapper_happy_path_returns_annotation_result(
    ...
    from phoenix_audit_agent.phoenix_tools import write_annotation as mod

    captured: dict = {}
    monkeypatch.setattr(mod, "AsyncClient", _fake_client_factory(captured))

    result = await mod.write_span_annotation(
        _SPAN_ID, 0.85, "tool returned 404", annotator="phoenix_audit_judge"
    )
    assert result.status == "ok"
    assert result.span_id == _SPAN_ID
    assert result.annotation_name == "phoenix_audit_cluster"
    assert result.score == 0.85
    assert captured["call_count"] == 1
    ann = captured["annotations"][0]
    assert ann["name"] == "phoenix_audit_cluster"
...
@pytest.mark.parametrize(
    ...
    [("phoenix_audit_judge", "LLM"), ("human", "HUMAN"), ("code", "CODE")],
)
...
async def test_wrapper_carries_service_version_in_metadata(
    ...
    """Annotation metadata carries `phoenix_audit_version` so server-side queries can filter."""
    from phoenix_audit_agent.phoenix_tools import write_annotation as mod
    ...
    assert metadata.get("phoenix_audit_version") == "1.2.3-test"
...
def test_build_annotation_rejects_invalid_annotator_kind_directly() -> None:
    ...
    from phoenix_audit_agent.config import get_settings as _gs
    from phoenix_audit_agent.phoenix_tools.write_annotation import _build_annotation
