...
``phoenix_audit.fault.type`` / ``phoenix_audit.fault.attack`` attributes and the
...
from phoenix_audit_agent.adk_types import CallbackContext, LlmRequest
from phoenix_audit_agent.injector.faults import PromptInjectionFault
...
async def test_instruction_override_lands_in_llm_input_value(
    ...
    assert span.attributes.get("phoenix_audit.fault.type") == "prompt_injection"
    assert span.attributes.get("phoenix_audit.fault.attack") == "instruction_override"
...
async def test_role_hijacking_lands_in_llm_input_value(
    ...
    span = _last_llm_span(exporter)
    assert span.attributes is not None
    assert span.attributes.get("phoenix_audit.fault.attack") == "role_hijacking"
...
async def test_payload_smuggling_lands_in_llm_input_value(
    ...
    span = _last_llm_span(exporter)
    assert span.attributes is not None
    assert span.attributes.get("phoenix_audit.fault.attack") == "payload_smuggling"
...
async def test_indirect_injection_lands_in_llm_input_value(
    ...
    span = _last_llm_span(exporter)
    assert span.attributes is not None
    assert span.attributes.get("phoenix_audit.fault.attack") == "indirect_injection"
...
async def test_user_role_not_found_leaves_contents_intact(
    ...
    assert span.attributes.get("phoenix_audit.fault.injected") is False
...
async def test_injected_attr_is_true_when_user_message_present(
    ...
    span = _last_llm_span(exporter)
    assert span.attributes is not None
    assert span.attributes.get("phoenix_audit.fault.injected") is True
