...
from phoenix_audit_agent.injector.faults import ContextPoisoningFault
from phoenix_audit_agent.injector.faults.context_poisoning import POISON_MARKER
...
async def test_retriever_insert_lands_poison_at_document_index_zero_list(
    ...
    assert span.attributes.get("phoenix_audit.fault.type") == "context_poisoning"
    assert span.attributes.get("phoenix_audit.fault.mode") == "retriever_insert"
...
async def test_history_insert_prepends_poisoned_message(
    ...
    assert span.attributes is not None
    assert span.attributes.get("phoenix_audit.fault.type") == "context_poisoning"
    assert span.attributes.get("phoenix_audit.fault.mode") == "history_insert"
...
async def test_retriever_patch_raises_on_unsupported_result_type() -> None:
    ...
    assert s.attributes.get("phoenix_audit.fault.injected") is False
