...
default; run locally with `GITLAB_TOKEN` + `GITLAB_TEST_PROJECT_ID` set:
    ...
    GITLAB_TOKEN=glpat-... GITLAB_TEST_PROJECT_ID=abu-phoenix-audit/test-target \\
        uv run pytest apps/phoenix-audit-agent/tests/integration/test_gitlab_emitter_online.py \\
        -v -m online
...
async def test_online_emit_returns_real_mr_url(
    ...
    assert result.branch_name.startswith("phoenix-audit/recipe-")
...
async def test_online_emit_commits_recipe_markdown_and_diff_files(
    ...
    async with httpx.AsyncClient(timeout=15.0) as http:
        # List tree at the branch tip — flat structure under `phoenix-audit/`.
        tree_response = await http.get(
            f"https://gitlab.com/api/v4/projects/{project_id_enc}/repository/tree",
            params={"ref": result.branch_name, "recursive": "true", "path": "phoenix-audit"},
            headers=headers,
        )
        tree_response.raise_for_status()
        paths = {item["path"] for item in tree_response.json()}

    assert f"phoenix-audit/patches/{recipe.recipe_id}.md" in paths
    assert any(
        p.startswith(f"phoenix-audit/patches/diffs/{recipe.recipe_id}_") and p.endswith(".diff")
        for p in paths
    )
    assert f"phoenix-audit/regression_tests/{recipe.recipe_id}.json" in paths
...
async def test_online_emit_mr_description_visible_on_gitlab(
    ...
    assert "phoenix-audit" in mr_body.get("labels", [])
