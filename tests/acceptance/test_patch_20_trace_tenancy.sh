...
# ANTI-anchor: refuse every affirmative Model-A wording. The negation
# above uses bare-verb form ("centralize"); any affirmative regression
# uses an inflected form. Listing all common verb inflections + modals
# makes the gate hostile to silent reversion.
assert_no_grep '(Phoenix )?Audit (centralizes|will centralize|may centralize|should centralize|must centralize|is centralizing|does centralize|DOES centralize)' docs/architecture.md
pass "ADR-013 (trace tenancy = customer-side Phoenix) lands in architecture.md; Model A wording refused in all forms"
...
# AND the bare form must NOT appear in architecture.md OUTSIDE the explicit
# tradeoff paragraph that contrasts the two. Easier check: no double-backtick
# `audit_run_id` reference where the namespaced form would be expected.
assert_no_grep 'filtered by `audit_run_id` span attribute' docs/architecture.md
pass "Filter attribute consistently namespaced as phoenix_audit.audit_run_id (no bare-form divergence)"
...
# invalidate the citation; this assertion catches that.
test "$(awk 'NR==29' research/google-cloud-rapid-agent/RAT-2-results.md | grep -cE 'Test 1' || true)" -ge 1 \
  || fail "RAT-2-results.md line 29 no longer starts Test 1 — fix architecture.md citation"
test "$(awk 'NR>=29 && NR<=49 {print}' research/google-cloud-rapid-agent/RAT-2-results.md | grep -cE '1\.37s' || true)" -ge 1 \
  || fail "RAT-2-results.md lines 29-49 no longer contain the 1.37s measurement — fix citation"
pass "ADR-013 cites RAT-2 Test 1 + 1.37s measurement verified in cited file + cited line range (29-49) verified"
...
pass "Namespaced phoenix_audit.audit_run_id is the v1 mitigation; HMAC properly tracked as TBD-18 and disclosed as non-delivery"

# -- Report-page paragraph locked VERBATIM as a canonical fixture -------------
# Round-2 silent-failure F2 + test-analyzer H1: fragment-pinning let the
# specific "derived embeddings" weakening (which the Verbatim-lock rationale
# explicitly warns against) slip past. Now using assert_block_present to
# pin the entire fixture string as a contiguous match.
#
# IMPORTANT: this is the canonical fixture from the schema's fenced text
# block, not the markdown blockquote (the blockquote rewraps on edit;
# the fenced text block is byte-stable). Using `read -r -d ''` because
# bash's `$(cat <<'EOF'...EOF)` pattern mishandles literal apostrophes
# in the heredoc body in some bash versions.
IFS= read -r -d '' COVER_FIXTURE <<'EOF' || true
...
# Special-case: "does not retain copies" is the EXACT weakening reviewer
# named. Reject it but only outside the Verbatim-lock rationale paragraph
# (which mentions it as a NEGATIVE example). Use a tighter sub-string that
# would only appear if the cover paragraph itself was weakened.
assert_no_grep 'cover-page .*does not retain copies' docs/run-config-schema.md
...
pass "Cover-page paragraph locked VERBATIM as canonical fixture + all named weakenings anti-anchored + rationale body locked"
...
# Also apply the Model A anti-anchor to PRD + schema + audit-notes.
for f in docs/PRD.md docs/run-config-schema.md docs/audit-notes.md; do
  assert_no_grep '(Phoenix )?Audit (centralizes|will centralize|may centralize|should centralize|must centralize|is centralizing|does centralize|DOES centralize)' "$f"
done
pass "PRD retains customer-tenancy + KMS anchors; Model A anti-anchor applied to all 3 spec files + PRD"
...
# transparent (round-2 silent-failure F10).
for d in "${SCAN_DIRS[@]}"; do
  file_count=$(find "$d" -name '*.py' -type f 2>/dev/null | wc -l | tr -d ' ')
  echo "  §14 scan target: $d ($file_count Python files)"
done
assert_no_pattern_in_dirs '(mock|fake|dummy|hardcoded|simulated)' --exclude-pattern '§14 carve-out' "${SCAN_DIRS[@]}"
pass "§14 clean: no mocks/fakes/dummies/hardcoded/simulated in ${SCAN_DIRS[*]} (helper applies B3/B4 silent-failure discipline)"
