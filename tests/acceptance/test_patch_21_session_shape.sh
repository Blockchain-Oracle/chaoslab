...
pass "6-probe session-mode mix locked VERBATIM via HARDCODED canonical literal (byte-drift fails loud)"
...
# Refuse any line claiming the 6-test battery is comprehensive, complete,
# exhaustive, full-coverage, or production-grade-coverage. EXCEPT lines that
# explicitly negate the claim (e.g., "NOT comprehensive coverage").
set +o pipefail
# Match overclaim, then strip ANY line that contains a negation/denial
# anywhere (NOT/not/cannot/never/no/MUST NOT/should NOT/treat ... as floor).
# Realistic risk: someone adds an AFFIRMATIVE overclaim sentence — those
# don't naturally include negations. Convoluted English ("this is fully
# comprehensive but not enough") could false-negative, accepted tradeoff.
overclaim_bad=$(grep -niE '(comprehensive|complete|exhaustive|full)\s+(coverage|audit|test|battery|sweep|sample)' docs/session-shape.md \
  | grep -ivE '(NOT|not|cannot|never|treat.*floor|deliberate.*tradeoff)' \
  | wc -l | tr -d ' ' || true)
set -o pipefail
test "$overclaim_bad" -eq 0 \
  || fail "session-shape.md contains $overclaim_bad over-claim(s) of comprehensiveness outside the negation context — Patch #21 is a deliberate tradeoff, NOT a comprehensive sweep"
pass "Comprehensiveness overclaims anti-anchored (deliberate-tradeoff framing preserved)"
